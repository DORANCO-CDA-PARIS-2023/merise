...
-----------------------------------------------
--Affichez le nom et le prénom de l'auteur de chaque livre
...
-----------------------------------------------
--Affichez les étudiants ayant emprunté plus de trois livres
SELECT s.name, s.firstname, COUNT(br.id) AS total_borrows
FROM student s
...
-----------------------------------------------
--Affichez les livres, les auteurs et les emprunts associés pour un étudiant spécifique (par exemple, l'étudiant avec l'ID 2)
SELECT b.title, a.name AS author_name, a.firstname AS author_firstname, br.Date_borrow, br.Date_back_schedulde, br.Date_back
FROM book b
JOIN author a ON b.id_author = a.id
JOIN borrow br ON b.id = br.id_book
